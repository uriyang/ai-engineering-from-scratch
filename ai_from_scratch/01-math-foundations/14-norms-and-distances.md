...
벡터나 데이터 두 개가 얼마나 가까운지를 재는 방법이 거리 함수입니다. 딥러닝과 머신러닝에서는 이 “가까움”의 정의가 작업마다 다릅니다.
...
특징 선택이 필요할 때
L1 정규화는 일부 가중치를 0으로 만들어 희소성을 만든다.
...
L2 정규화(Ridge)는 가중치가 너무 커지지 않게 누르는 역할을 합니다. L1과 달리 0으로 딱 떨어뜨리기보다 전체를 부드럽게 줄입니다.
...
Lp 노름은 p 값이 커질수록 모양이 점점 네모에 가까워집니다. L1은 꼭짓점이 도드라지고, L-infinity는 축에 평행한 정사각형이 됩니다.
...
- 크기가 노이즈이고 방향이 신호인 모든 영역
- 추천 시스템(사용자 선호도 벡터)
- 임베딩 검색(벡터 데이터베이스는 거의 항상 코사인 또는 내적을 사용함)
...
whitening = 데이터의 스케일을 맞추고, 서로 얽힌 축을 풀어주는 전처리입니다.
...
이 식은 P와 Q가 얼마나 다른지를 보여줍니다. 하지만 KL 발산은 대칭이 아니어서 `D_KL(P || Q) != D_KL(Q || P)`입니다.
...
즉, 거리 함수는 단순한 수식이 아니라 “무엇을 비슷하다고 볼 것인가”를 정하는 기준입니다. 이 기준이 바뀌면 이웃도, 손실도, 정규화도 달라집니다.
