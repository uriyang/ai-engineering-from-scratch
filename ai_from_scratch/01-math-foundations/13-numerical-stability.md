...
수치적 안정성은 계산이 너무 커지거나 너무 작아져서 망가지지 않게 만드는 방법이다. 딥러닝에서는 loss가 `NaN`이 되거나 gradient가 0이 되거나, 부동소수점 오차가 쌓여 학습 결과가 달라지는 일이 자주 생긴다.

## IEEE 754: 부동소수점

- 컴퓨터는 실수를 정확히 저장하지 못하고 근사값으로 저장한다
- float는 부호(sign), 지수(exponent), 가수(mantissa)로 나뉜다
- 지수는 범위를, 가수는 정밀도를 결정한다
...
실무에서는:
- float를 비교할 때는 `==`보다 epsilon을 두고 비교하는 편이 안전하다
